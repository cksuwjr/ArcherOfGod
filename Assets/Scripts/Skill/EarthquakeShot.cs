...
public class EarthquakeShot : Skill
{
    ...
    private IEnumerator Cast()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            var arrow = PoolManager.Instance.earthquakeArrowPool.GetPoolObject();
            if (arrow.TryGetComponent<Arrow>(out var arrowComponent))
            {
                arrowComponent.Init(gameObject, GetComponent<PlayerController>().enemy, 100 + Random.Range(0, 100), 0f);
                if (TryGetComponent<AttackManager>(out var attackManager))
                {
                    ...
                    arrow.GetComponent<Rigidbody2D>().velocity =
                        BasicAttack.GetLaunchVelocity_ByApex(
                            ...
                            GetComponent<PlayerController>().enemy.transform.position + Vector3.right * Random.Range(-2f, 2f),
                            Mathf.Abs(GetComponent<PlayerController>().enemy.transform.position.x - transform.position.x));
